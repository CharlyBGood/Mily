...
export default function PDFExportButton({
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      if (onBeforePdfExport) {
        contentElement = await onBeforePdfExport()
      }
      ...
      for (let i = 0; i < groupedMeals.length; i++) {
        ...
        for (let j = 0; j < group.meals.length; j++) {
          ...
          if (meal.photo_url && contentElement) {
            try {
              // Find the image in the content element
              const mealId = meal.id
              const mealElement = contentElement.querySelector(`[data-meal-id="${mealId}"]`)
              const imgElement = mealElement?.querySelector("img")
              ...
              if (imgElement) {
                ...
                pdf.addImage(imgData, "JPEG", pageMargin + 5, yPosition, photoWidth, photoHeight, undefined, "FAST")
  ...
  return (
    <Button
      ...
      disabled={disabled || meals.length === 0 || isGenerating}
